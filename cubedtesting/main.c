...
int	ft_checkcollision(t_vars *vars, double x, double y, float angle)
{
	...
	if (vars->map[(int)(collision_x / 100)][(int)(collision_y / 100)] == 1)
		return (0);
	return (1);
}
...
int	ft_loopcolor(char *buff, int i)
{
...
void	ft_insertcolor(unsigned int *color, int c[3], char *buff)
{
...
int	ft_putcolor(unsigned int *color, char *buff, int i)
{
	char		*tmp;
	int			c[3];
	int			j;
...
int	ft_parsevars(t_vars *vars, char *buff)
{
	...
	while (buff[i])
	{
		if (buff[i] == 'N' && buff[i + 1] == 'O' && buff[i + 2] == ' ')
			...
			i = ft_putcolor(&vars->floorcolor, buff, i + 2);
		else if (buff[i] == 'C' && buff[i + 1] == ' ')
			i = ft_putcolor(&vars->ceilingcolor, buff, i + 2);
		else if (buff[i] == '\n')
			i++;
		else
			return (i);
...
void	ft_allocloop(t_vars *vars, char *buff, int rows, int i, int len)
{
	while (buff[i])
	{
		...
		if (buff[i] == '\n')
		{
			...
			if (buff[i + 1] == '\0')
				len++;
			vars->map[rows] = (int *)ft_calloc(sizeof(int), len + 1);
			len = 0;
			rows++;
		}
		len++;
...
void	ft_allocmap(t_vars *vars, char *buff, int start)
{
	...
	vars->map = (int **)ft_calloc(sizeof(int *), rows);
	len = 0;
	rows = 0;
