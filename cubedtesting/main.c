...
int	ft_exitcross(t_vars *v)
{
	ft_exit(v, "Bye bye\n");
	return (0);
}

void	ft_key2(int keycode, t_vars *vars)
{
	if (keycode == 0x0061)
		if (ft_checkcollision(vars, vars->player.pos.x, \
		vars->player.pos.y, vars->player.dir - 90))
			ft_movepoint(&vars->player.pos.x, \
			&vars->player.pos.y, vars->player.dir - 90, 50);
	if (keycode == 0x0064)
		if (ft_checkcollision(vars, vars->player.pos.x, \
		vars->player.pos.y, vars->player.dir + 90))
			ft_movepoint(&vars->player.pos.x, \
...
int	ft_key(int keycode, t_vars *vars)
{
	...
	if (keycode == 0x0073)
		if (ft_checkcollision(vars, vars->player.pos.x, \
		vars->player.pos.y, vars->player.dir - 180))
			ft_movepoint(&vars->player.pos.x, \
			&vars->player.pos.y, vars->player.dir - 180, 50);
	ft_key2(keycode, vars);
	return (0);
}
...
void	ft_put_image(t_vars *vars, t_disvars *d)
{
...
void	ft_display(t_vars *vars, t_ray *ray, float raynum)
{
	...
	while (d.j < d.y_end)
	{
		d.color = *(unsigned int *)(d.img->addr + (int)((int) \
		d.imgy * d.img->line_length) + (int)ray->imgx);
		mlx_pixel_put(vars->mlx, vars->win, d.screenx, d.j, d.color);
		mlx_pixel_put(vars->mlx, vars->win, d.screenx + 1, d.j, d.color);
		d.imgy += d.yadd;
		d.j++;
	}
	ft_put_image(vars, &d);
...
void	ft_checkhit180(t_ray *ray, int **worldMap)
{
	...
	else if (worldMap[(int)(ray->pos.y) / \
	100][(int)(ray->pos.x - 1) / 100] != 2)
	{
		ray->hit = 1;
		ray->orientation = 3;
		ray->imgx = 100.0 - fmod(ray->pos.x, 100.0);
...
void	ft_checkhit270(t_ray *ray, int **worldMap)
{
	...
	else if (worldMap[((int)ray->pos.y - 1) / \
	100][((int)ray->pos.x) / 100] != 2)
	{
		ray->hit = 1;
		ray->orientation = 1;
...
void	ft_rayloop(t_ray *ray, int **map)
{
	if (ray->dir < 90)
		ft_ray90(ray, map);
	else if (ray->dir < 180)
...
char	*substring(char *str, int start, int end)
{
	...
	sub = (char *)ft_calloc(length + 1, sizeof(char));
	i = 0;
	while (i < length)
	{
		sub[i] = str[start + i];
...
void	ft_checkparse(t_vars *v, char *buff)
{
	if (v->north.img == NULL || \
	v->south.img == NULL || v->east.img == NULL || \
	v->west.img == NULL)
		ft_exit(v, "Missing texture\n");
	if (v->floorcolor == 0 || v->ceilingcolor == 0)
		ft_exit(v, "Missing color\n");
}
...
void	ft_allocloop(t_vars *vars, char *buff, int rows, int i)
{
	int	len;
...
void	ft_floodmanage(t_vars *vars)
{
	int	i;
	int	j;
...
int	main(int argc, char **argv)
{
	...
	v.ceiling.addr = mlx_get_data_addr(v.ceiling.img, \
	&v.ceiling.bits_per_pixel, &v.ceiling.line_length, &v.ceiling.endian);
	v.floor.addr = mlx_get_data_addr(v.floor.img, \
	&v.floor.bits_per_pixel, &v.floor.line_length, &v.floor.endian);
	fill_image_with_color(&v.ceiling, v.ceilingcolor);
	fill_image_with_color(&v.floor, v.floorcolor);
	mlx_hook(v.win, 2, 1L << 0, ft_key, &v);
	mlx_hook(v.win, 17, 1L << 17, ft_exitcross, &v);
	mlx_loop_hook(v.mlx, ft_rays, &v);
	mlx_loop(v.mlx);
	return (0);
}
